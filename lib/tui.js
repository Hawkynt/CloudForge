...
function phaseBanner(phaseName, subTask, totalSubTasks, iteration, maxIterations, tokens, sessionStart) {
  ...
  const lines = [
    ...
    `${DIM} Iteration:${RESET} ${sessionIter}/${sessionBudget} (total: ${iteration}) ${DIM}|${RESET} ${DIM}Tokens:${RESET} ${tokenIn} in / ${tokenOut} out`,
    `${DIM}${'─'.repeat(50)}${RESET}`,
  ];
  process.stdout.write(lines.join('\n') + '\n');
}
...
function phaseProgressLine(allPhases, completedPhases, currentPhase) {
  ...
  const markers = allPhases.map((name) => {
    if (completedPhases.includes(name)) return `${GREEN}#${RESET}`;
    if (name === currentPhase) return `${YELLOW}${BOLD}>${RESET}`;
    return `${DIM}-${RESET}`;
  });

  const position = idx >= 0 ? `phase ${idx + 1}/${total}` : `phase ?/${total}`;
  process.stdout.write(` ${DIM}[${RESET}${markers.join('')}${DIM}]${RESET}  ${DIM}${position}${RESET}\n`);
}
...
function haltMessage(reason) {
  ...
  const lines = [
    ...
    `${RED} ${reason}${RESET}`,
    `${DIM} State saved to .cloudforge/state.json - re-run without arguments to auto-resume${RESET}`,
    '',
  ];
  process.stdout.write(lines.join('\n') + '\n');
}
...
function finalSummary(state) {
  ...
  const lines = [
    ...
    `${DIM} Iterations:${RESET} ${state.iteration}/${state.maxIterations} (session: ${state.iteration - (state.sessionStartIteration || 0)})`,
